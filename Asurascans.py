...
class AsurascansScraper(MangaScraper):
    ...
    def fetch_updates_from_page(self, page_number: int = 1):
        ...
        # Navigate to the URL
        page.goto(self.base_url + self.base_latest + str(page_number), wait_until="domcontentloaded")
    ...
    def fetch_manga_details(self, manga_url):
        ...
        for link in links:
            url = link['href']
            if "author" in url:
                author.append(url.split('=')[-1].replace("%20", " ")) # only if one author ( need to check how url looks like if multiple authors )
            elif "artist" in url:
                artist.append(url.split('=')[-1].replace("%20", " ")) # only if one author ( need to check how url looks like if multiple authors )
            elif "genres" in url:
                genre = url.split('=')[-1].replace("%20", " ")
                genres.append(genre)
            elif "chapter" in url:
...
if __name__ == "__main__":
    ...
    """ result = scraper.fetch_latest_updates()
    print(result) """
    """ result = scraper.fetch_manga_details("https://asurascans.com/comics/mount-hua-sects-genius-phantom-swordsman-030ff47a")
    print(result) """
    scraper.scrape()
